...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin{
  ...
  List<CourseModel> bundleData = [];
    ...
    bool isLoadingCategoriesData=false;
...
List<CategoryModel> subSubSubCategories = [];
String? selectedSubSubSubCategoryId;
...
CategoriesService.categories().then((value) {
  print('subSubSubCategories Count: ${subSubSubCategories.length}');
  ...
  setState(() {
    ...
    // جلب السب كاتيجوريز من أول ليفيل
    if (selectedCategoryId != null) {
      final selectedCategory = value.firstWhere(
    ...
    // جلب السب سب كاتيجوريز من تاني ليفيل
    if (selectedSubCategoryId != null && subCategories.isNotEmpty) {
      final selectedSubCategory = subCategories.firstWhere(
        (cat) => cat.id.toString() == selectedSubCategoryId,
        orElse: () => CategoryModel(),
      );
      subSubCategories = selectedSubCategory.subCategories ?? [];
    }

    // جلب السب سب سب كاتيجوريز من ثالث ليفيل
    if (selectedSubSubCategoryId != null && subSubCategories.isNotEmpty) {
      final selectedSubSubCategory = subSubCategories.firstWhere(
        (cat) => cat.id.toString() == selectedSubSubCategoryId,
        orElse: () => CategoryModel(),
      );
      subSubSubCategories = selectedSubSubCategory.subCategories ?? [];
    }
  });
});
...
isLoadingCategoriesData
  ...
  children: [
    // const Text("Main Categories", style: TextStyle(fontWeight: FontWeight.bold)),
    CategoryCardList(
    ...
    CategoryCardList(
      ...
      onTap: (subCat) {
        setState(() {
          selectedSubCategoryId = subCat.id.toString();
          subSubCategories = subCat.subCategories ?? [];
        });
      },
    ),
    const SizedBox(height: 16),
    // const Text("Sub Sub Categories", style: TextStyle(fontWeight: FontWeight.bold)),
    CategoryCardList(
      categories: subSubCategories,
      selectedId: selectedSubSubCategoryId,
      onTap: (subSubCat) {
        setState(() {
         selectedSubSubCategoryId = subSubCat.id.toString();
    subSubSubCategories = subSubCat.subCategories ?? [];        
        nextRoute(FilterCategoryPage.pageName, arguments: subSubCat);
    });
      },
    ),
//     const Text("Sub Sub Sub Categories", style: TextStyle(fontWeight: FontWeight.bold)),

// if (selectedSubSubCategoryId != null && subSubSubCategories.isNotEmpty)
//   CategoryCardList(
//     categories: subSubSubCategories,
//     selectedId: selectedSubSubSubCategoryId,
//     onTap: (subSubSubCat) {
//       setState(() {
//         selectedSubSubSubCategoryId = subSubSubCat.id.toString();
//         nextRoute(FilterCategoryPage.pageName, arguments: subSubSubCat);
//       });
//     },
//   )
// else
//   const SizedBox()// أو Loading، أو نص "لا توجد بيانات"
...
class SocialLinksRow extends StatelessWidget {
  ...
  Widget buildButton(IconData icon, String label, Color color, String url) {
    return Column(
      children: [
        IconButton(
      ...
      child: Row(
        ...
        children: [
          buildButton(
            ...
            'Facebook',
            Colors.blue[800]!,
            'https://www.facebook.com/share/15YQVgVrDQ/',
          ),
          buildButton(
            FontAwesomeIcons.whatsapp,
            'WhatsApp',
            Colors.green[600]!,
            'https://wa.me/201551275851',
          ),
          buildButton(
            FontAwesomeIcons.youtube,
            'YouTube',
            Colors.red,
            'https://youtube.com/channel/UC4L442Y2kv-fhQrDya4MK5w?feature=shared',
          ),
        ],
      ),
    );
