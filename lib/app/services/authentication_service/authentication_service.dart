...
class AuthenticationService {
  ...
  static Future login(String username, String password) async {
    ...
    try {
      ...
      print('Login URL: $url');
      print('Constants.baseUrl: ${Constants.baseUrl}');
      print('Constants.dommain: ${Constants.dommain}');
      print('Constants.apiKey: ${Constants.apiKey}');
      ...
      print(
          'Response body preview: ${res.body.length > 200 ? res.body.substring(0, 200) + "..." : res.body}');
      ...
      try {
        ...
        print('JSON structure: ${jsonResponse.keys.toList()}');
        print('Success field: ${jsonResponse['success']}');
        print('Data field exists: ${jsonResponse.containsKey('data')}');
        if (jsonResponse.containsKey('data')) {
          print('Data fields: ${jsonResponse['data'].keys.toList()}');
        }
      } catch (jsonError) {
      ...
      if (jsonResponse['success']) {
        print('=== SUCCESS LOGIN DEBUG ===');
        print('Token exists: ${jsonResponse['data'].containsKey('token')}');
        print('User ID exists: ${jsonResponse['data'].containsKey('user_id')}');

        await AppData.saveAccessToken(jsonResponse['data']['token']);
        print('Token saved successfully');

        print('User ID: ${jsonResponse['data']['user_id']}');
        print("jsonResponse['data']['user_id']");

        await AppData.saveEmail(username);
        print('Email saved: $username');

        await AppData.saveName('');
        print('Name saved (empty)');
        ...
        print('Error message: ${jsonResponse['message'] ?? 'No message'}');
        print('Error code: ${jsonResponse['code'] ?? 'No code'}');
