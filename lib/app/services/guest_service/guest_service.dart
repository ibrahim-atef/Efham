...
class GuestService{
  ...
  static Future config()async{
    try{
      ...
      log("GuestService config jsonResponse${jsonResponse}");
      if(res.statusCode == 200){
        
        PublicData.apiConfigData = jsonResponse;
        print(jsonResponse);
        print("GuestService config jsonResponse");
  ...
  static Future<RegisterConfigModel?> registerConfig(String role)async{
    try{
      ...
      print('Calling registerConfig for role: $role');
      print('URL: $url');
      ...
      if(res.statusCode == 200){
        ...
        if (jsonResponse['data'] != null) {
          ...
          var config = RegisterConfigModel.fromJson(jsonResponse['data']);
          print('Successfully created RegisterConfigModel');
          return config;
        } else {
      ...
      log('Error in registerConfig(): $e');
      if (e is TimeoutException) {
        print('Register config API timeout');
      }
      return null;
    }
  }
